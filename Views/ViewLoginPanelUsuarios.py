...
from .ViewCrudUsuariosTpv import ViewCRUDUsuarios
...
class View_Login:
    ...
    def __init__(self):
        ...
        self.window_login.title(
            f"Login Panel de administracion de usuarios -> {self.instance_config.get_config('general', 'informacion_estacionamiento', 'nombre_estacionamiento')}")
    ...
    def get_data(self, user=None, password=None):
        ...
        data = self.instance_config.get_config('funcionamiento_interno', 'panel_usuarios')
        ...
        if self.success_access is not None:
            message = f'[{hour}]- El usuario [{actual_user}] {"accedio" if self.success_access else "intento acceder"} al panel de configuracion de usuarios.'
