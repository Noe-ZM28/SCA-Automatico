...
class ViewPaidPension:
    def __init__(self, id_pension) -> None:
        
        self.id_pension = id_pension

        # Crea la ventana principal
        self.window_paid_pension = tk.Toplevel()
        # instancia de la clase de herramientas
        self.instance_tools = Tools()
        self.instance_config = ConfigController()
        self.model_pensinados = Pensionados()
        self.printer_controller = PrinterController()

        # se elimina la funcinalidad del boton de cerrar
        self.window_paid_pension.protocol(
            "WM_DELETE_WINDOW", lambda: self.instance_tools.desconectar(self.window_paid_pension))
        # # deshabilita los botones de minimizar y maximizar
        # self.window_paid_pension.attributes('-toolwindow', True)
        # # coloca la ventana al frente de otras ventanas
        # self.window_paid_pension.attributes('-topmost', True)

        ancho_max = self.window_paid_pension.winfo_screenwidth()
        alto_max = self.window_paid_pension.winfo_screenheight()

        self.window_paid_pension.resizable(False, False)
        self.get_config_data()

        self.window_paid_pension.title(f'Panel de administracion reporte dias')

        # Configura la columna principal del panel para que use todo el espacio disponible
        self.window_paid_pension.columnconfigure(0, weight=1)
        ...
        self.window_paid_pension.mainloop()
    ...
    def view_campos_consulta(self):
        ...
        seccion_superior = tk.LabelFrame(self.window_paid_pension, text='')
        ...
        seccion_informacion = tk.LabelFrame(
            self.window_paid_pension, text='Seleccione el mes a consultar', font=self.font_tittle_system)
        ...
        seccion_botones.grid(row=1, column=0, padx=5, pady=5)

        self.login_button = tk.Button(
            seccion_botones, text="Imprimir", command=self.print_paids, width=10, height=1, anchor="center", background=self.button_color, fg=self.button_letters_color, font=self.font_botones_interface)
        self.login_button.grid(column=0, row=0, padx=5, pady=5)

        self.exit_button = tk.Button(
            seccion_botones, text="Cancelar", command=lambda: self.instance_tools.desconectar(self.window_paid_pension), width=10, height=1, anchor="center", background=self.button_color, fg=self.button_letters_color, font=self.font_botones_interface)
        self.exit_button.grid(column=1, row=0, padx=5, pady=5)



    def print_paids(self):
        try:
            year_param = self.variable_años.get()
            month_param = self.lista_desplegable_meses.current() + 1

            registros = self.model_pensinados.get_list_paids_pensiones(year_param, month_param, self.id_pension)
            
            if len(registros) ==0:
                raise SystemError("No hay registro de pago para la fecha indicada.\n\nSi consideras que se trata de un error ponte en contacto con un administrador inmediatamente")
            
            for registro in registros:
                print(registro)

                id, Nom_cliente, Apell1_cliente, Apell2_cliente, fecha_pago, vigencia, monto, usuario, tipo_pago = registro

                state = self.printer_controller.print_pension_pay_ticket(
                    f"Pension-{self.nombre_estacionamiento}-{id}", Nom_cliente, Apell1_cliente, Apell2_cliente, fecha_pago, vigencia, monto, usuario, tipo_pago, title = "Reimpresion de comprobante de pago")

                if state != None:
                    raise SystemError(state)
                
                self.instance_tools.desconectar(self.window_paid_pension)
